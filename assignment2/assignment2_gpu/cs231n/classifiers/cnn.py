...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # out.shape = (N, F, (H_in + 2 * pad - filter) // stride + 1, same but W_in)
        Hout = (input_dim[1] + 2 * pad - filter_size) // 2 + 1
        Wout = (input_dim[2] + 2 * pad - filter_size) // 2 + 1

        self.params['W2'] = np.random.normal(
            scale=weight_scale,
            size=(num_filters * Hout * Wout, hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        dout, grads['W3'], grads['b3'] = affine_backward(df, cache3)
        ...
        loss += self.reg * (np.sum(W3 ** 2) + np.sum(W2 ** 2) + np.sum(W1 ** 2))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
